...
class optStruct:
    def __init__(self, dataMatIn, classLabels, C, toler):
        ...
        self.b = 0
        self.eCache = mat(zeros((self.m, 2)))     # 误差缓存，第一列是有效的标志位，第二列是实际E值
...
def smoSimple(dataMatIn, classLabels, C, toler, maxIter):
    """Repeat till convergence {
        1. Select some pair {alpha_i, alpha_j} to update next (using a heuristic that
            tries to pick the two that will aloow us to make the biggest progress
            towards the global maximum);
        2. Reoptimize W(alpha) with respect to alpha_i and alpha_j, while holding all
            the other alpha_k's (k != i,j) fixed.
    }
    """
